...
document.getElementById("mbtn2").style.borderBottomColor = "#06c";	
document.getElementById("mbtn3").style.borderBottomColor = "#14A6FA";		
}
...
<a href="packages_received.php"><div class="member_buttons" id="mbtn3" onmouseover="javascript:changecolor3();" onmouseout="javascript:backtomain3();" style="border-bottom-color:#06c;">PACKAGES RECEIVED</div></a>
<div class="member_buttons" id="mbtn4" style="background-color:#06c; border-bottom-color:#06c; color:#fff;">PACKAGES SENT</div> 
...
if($status == "active"){
		  $sql1 = "SELECT * FROM  `packages_delivered` WHERE email = '$memberemail'";
		  $results1 = mysql_query($sql1,$connection) or die(mysql_error());
		  $total = mysql_num_rows($results1);
 		  if($total >6){$changestyle = "style='overflow-y:scroll; height:270px;'";}else{$changestyle = '';}
		  
		   
		  if($total == ''){
					echo "<div id='packages_expected_msg_frame'>";
                    echo "<input  type='text' style='background-color:#ECECEC; border:0px;' value='NO PAID PACKAGES TO DELIVER.' class='p_e_texture' readonly/>";
                    echo "</div>";
 ...
 ?>
          ...
          <div id="packages_expected_top"><div class="packages_expected_titles_5">TRACKING NO.</div><div class="packages_expected_titles_6">DATE DELIVERED</div></div>
	...
	for($j=0; $j<$total; $j++){
		 ...
		 $delivery_date = $row1['date_delivered'];
	
	?>      
          
          <div class="packages_delivered_row">
          <div class="packages_expected_row_left"><?php echo $j+1; ?></div>
          <div class="packages_delivered_row_mid" ><?php echo $tracking[$j]; ?></div>
          <div class="packages_delivered_row_right" ><?php echo  $delivery_date; ?></div>
          </div>
